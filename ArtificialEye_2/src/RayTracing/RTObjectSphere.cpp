...
bool ee::RTObjectSphere::calcIntersection(Ray ray, int ignoreTriangle, bool towardsPhoto) const
{
	...
	const glm::vec3 inv_dir = glm::vec3(m_invPosition * glm::vec4(ray.dir, 0.f)); // the direction is the one that is provided
	const glm::vec3 inv_origin = glm::vec3(m_invPosition * glm::vec4(ray.origin, 1.f)); // the origin is at the position of the sphere
